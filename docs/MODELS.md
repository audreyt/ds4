...
| `preferred` | Vision-Exp abliterated IQ2 + encoder, about 81.63 GiB; fork default |
| `headroom128` | Legacy abliterated 0731 Flash, about 81 GiB, no vision |
...
On this fork, `preferred` (alias `ds4f-vision-abliterated`) is the default:
Vision-Exp abliterated IQ2 plus its encoder
(`audreyt/DeepSeek-V4-Flash-Vision-Exp-Abliterated-GGUF` @ `367a1fef`,
80.76 GiB language + 0.89 GiB encoder). It links `./ds4flash.gguf` to the
language GGUF; pass `--vision gguf/DeepSeek-V4-Flash-Vision-Encoder.gguf`
at runtime. Legacy `headroom128` is still available but is not Vision-Exp;
do not pass `--vision` with it.
...
On this fork the abliterated Vision-Exp IQ2 is also available and is the
`preferred` default:
