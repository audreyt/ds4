...
// Q8_0 token-embedding gather: dequantize blocks while reading the row.
// Source row is `(ne00 / QK8_0)` `block_q8_0` records ([half scale][int8 qs[32]]);
// destination is `ne00` floats. Layout dispatch matches kernel_get_rows_f.
kernel void kernel_get_rows_q8_0(
    ...
    auto psrc = (device const block_q8_0 *) (src0 + i03*args.nb03 + i02*args.nb02 + r*args.nb01);
    auto pdst = (      device float       *) (dst  + i12*args.nb3  + i11*args.nb2  + i10*args.nb1);

    for (int ind = iw0*ntg.x + tiitg; ind < args.ne00t;) {
        const int blk = ind / QK8_0;
        const int idx = ind - blk * QK8_0;
        const float d = (float) psrc[blk].d;
        pdst[ind] = (float) psrc[blk].qs[idx] * d;
