...
static void test_metal_f16_mpp_experimental_matmul(void) {
    ...
    for (uint32_t o = 0; o < out_dim; o++) {
        for (uint32_t i = 0; i < in_dim; i++) {
            float w = (float)((int)((o * 7u + i * 11u) % 37u) - 18) / 96.0f;
            weights[(uint64_t)o * in_dim + i] = test_float_to_f16(w);
        }
    }
    ...
    for (uint32_t t = 0; t < n_tok; t++) {
        for (uint32_t i = 0; i < in_dim; i++) {
            x_host[(uint64_t)t * in_dim + i] =
                (float)((int)((t * 13u + i * 17u) % 41u) - 20) / 64.0f;
        }
    }
    ...
    TEST_ASSERT(ds4_metal_set_model_map(weights_raw, weight_alloc) != 0);
    ds4_metal_set_quality(false);
    TEST_ASSERT(ds4_metal_matmul_f16_tensor(out_ref, weights_raw, weight_alloc, 0,
                                            in_dim, out_dim, x, n_tok) != 0);

    int have_mpp = ds4_metal_matmul_f16_mpp_experimental_tensor(
        out_mpp, weights_raw, weight_alloc, 0, in_dim, out_dim, x, n_tok);
    if (!have_mpp) {
        fprintf(stderr, "ds4-test: skipping experimental MPP F16 matmul; Metal 4 tensor API unavailable\n");
        free(x_host);
        free(ref_host);
        free(mpp_host);
        ds4_metal_tensor_free(x);
    ...
    if (max_abs >= 0.08f) {
        fprintf(stderr, "ds4-test: experimental MPP F16 matmul max_abs=%f at token=%llu out=%llu ref=%f mpp=%f\n",
                max_abs,
...
static void test_metal_int8_mpp_probe_matmul(void) {
    const uint32_t m = 32;
    ...
    ds4_metal_tensor *a = ds4_metal_tensor_alloc(a_bytes);
    ds4_metal_tensor *b = ds4_metal_tensor_alloc(b_bytes);
    ds4_metal_tensor *out = ds4_metal_tensor_alloc(out_bytes);
    TEST_ASSERT(a != NULL);
    TEST_ASSERT(b != NULL);
    TEST_ASSERT(out != NULL);
    if (!a || !b || !out) {
    ...
    int8_t *a_host = malloc((size_t)a_bytes);
    int8_t *b_host = malloc((size_t)b_bytes);
    int32_t *out_host = malloc((size_t)out_bytes);
    TEST_ASSERT(a_host != NULL);
    TEST_ASSERT(b_host != NULL);
    TEST_ASSERT(out_host != NULL);
    if (!a_host || !b_host || !out_host) {
        free(a_host);
        free(b_host);
        free(out_host);
        ds4_metal_tensor_free(a);
    ...
    for (uint32_t row = 0; row < m; row++) {
        for (uint32_t col = 0; col < k; col++) {
            a_host[(uint64_t)row * k + col] =
                (int8_t)((int)((row * 11u + col * 7u) % 31u) - 15);
        }
    }
    for (uint32_t row = 0; row < n; row++) {
        for (uint32_t col = 0; col < k; col++) {
            b_host[(uint64_t)row * k + col] =
                (int8_t)((int)((row * 5u + col * 13u) % 29u) - 14);
        }
    }

    TEST_ASSERT(ds4_metal_tensor_write(a, 0, a_host, a_bytes) != 0);
    TEST_ASSERT(ds4_metal_tensor_write(b, 0, b_host, b_bytes) != 0);

    int have_mpp = ds4_metal_int8_matmul_i32_mpp_probe_tensor(out, a, b, m, n, k);
    if (!have_mpp) {
        fprintf(stderr, "ds4-test: skipping INT8 MPP probe matmul; Metal 4 tensor API unavailable\n");
        free(a_host);
        free(b_host);
        free(out_host);
        ds4_metal_tensor_free(a);
    ...
    TEST_ASSERT(ds4_metal_tensor_read(out, 0, out_host, out_bytes) != 0);
    ...
    for (uint32_t row = 0; row < m; row++) {
        for (uint32_t col = 0; col < n; col++) {
            ...
            for (uint32_t kk = 0; kk < k; kk++) {
                ref += (int32_t)a_host[(uint64_t)row * k + kk] *
                       (int32_t)b_host[(uint64_t)col * k + kk];
            }
            const uint64_t idx = (uint64_t)row * n + col;
            int32_t err = out_host[idx] - ref;
            if (err < 0) err = -err;
            if (err > max_abs) {
                max_abs = err;
                max_index = idx;
    ...
    if (max_abs != 0) {
        fprintf(stderr, "ds4-test: INT8 MPP probe max_abs=%d at row=%llu col=%llu got=%d\n",
                max_abs,
                (unsigned long long)(max_index / n),
                (unsigned long long)(max_index % n),
                out_host[max_index]);
    }
    TEST_ASSERT(max_abs == 0);
...
static void test_metal_kernel_group(void) {
    ...
    test_metal_int8_mpp_probe_matmul();
}
